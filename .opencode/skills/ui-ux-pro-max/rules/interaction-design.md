...
```
Good: Cart → Shipping → Payment → Review → Confirmation
      [Back] [Back] [Back] [Edit]   Progress bar visible
      Each step validates · Review before commit

Bad:  Cart → Login Required → Shipping → Re-enter email → Payment → Surprise fees → Submit
      [No going back]
```
